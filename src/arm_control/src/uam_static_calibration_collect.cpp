...
// 默认静态采样姿态序列。
//
// 这里按真机机械结构约束把样本总数进一步扩成 36 个，用来做更全面的静态标定：
// 1. arm2 的正向最大测试角仍然限制在 +25 deg，不再继续测更大的正角，
//    避免正向抬臂过大时被机械结构挡住；
// 2. 正角侧分成 0 / +10 / +20 / +25 四层，每一层都增加 arm1 覆盖点，
//    提高对工作空间前后左右的几何约束；
// 3. 负角侧增加 -10 / -20 两层，用来补充“向下摆”一侧的信息，
//    这样拟合时不会只被正向样本主导；
// 4. 顺序不是按分层简单拼接，而是人工重排成相邻样本变化较小的路径，
//    尽量减少真机大幅反向跳变与超时。
std::vector<std::pair<double, double>> DefaultSamples() {
    return {
        ...
        // 第二层：arm2 = +10 deg，继续覆盖较宽的 arm1 范围。
        {60.0, 10.0},
        {40.0, 10.0},
        {20.0, 10.0},
        {0.0, 10.0},
        ...
        // 第三层：arm2 = +20 deg，提供中等正向抬臂下的主要几何约束。
        {-60.0, 20.0},
        {-40.0, 20.0},
        {-20.0, 20.0},
        {0.0, 20.0},
        ...
        // 第四层：arm2 = +25 deg，作为正向最大测试层，只保留较保守的 arm1 采样点。
        {45.0, 25.0},
        {20.0, 25.0},
        {0.0, 25.0},
        {-20.0, 25.0},
        ...
        // 第六层：arm2 = -20 deg，再补一层向下摆样本，但只保留居中的 arm1，降低结构干涉风险。
        {30.0, -20.0},
        {0.0, -20.0},
        {-30.0, -20.0},
    };
